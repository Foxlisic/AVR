...
public:
    ...
    void update_player(byte lvl[]) {
        update_sprite(player_x, player_y, lvl);
    }
    ...
    void level_select() {
        ...
        for (;;) {
            ...
            if      (ch == key_DN) current_level--;
            else if (ch == key_UP) current_level++;
            else if (ch == key_ENTER || ch == key_ESC) break;

            // Ограничитель
            if (current_level < 0)
                current_level = SOKOBAN_MAX_LEVEL - 1;
            else if (current_level >= SOKOBAN_MAX_LEVEL)
                current_level = 0;
        }
    }
    ...
    void main() {
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if      (ch == key_UP)      { movex =  0; movey = -1; }
                else if (ch == key_DN)      { movex =  0; movey =  1; }
                else if (ch == key_LF)      { movex = -1; movey =  0; dirmario = 1; }
                else if (ch == key_RT)      { movex =  1; movey =  0; dirmario = 0; }
                else if (ch == key_ENTER)   { break; }
                else if (ch == key_TAB)     { level_select(); break; }
