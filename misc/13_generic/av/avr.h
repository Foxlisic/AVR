...
protected:
    ...
    int width, height, frame_length;
    int pticks;
    int show_fps = 0, fps_latch = 0, fps = 0, fps_tick = 0;
    ...
    int     ds_dump_start;
    int     ds_counter = 0;
    int     pvsram[96];             // Для отладчика
    FILE*   fp_debuglog = NULL;

    // -----------------------------------------------------------------
    // Видеовывод и периферия
    // -----------------------------------------------------------------
...
public:
    ...
    int     fetch() {
        ...
        int data = program[pc] + program[pc + 1] * 256;
        pc = (pc + 2) & 0x1ffff;
        return data;
    }
    ...
    void    put16(int a, uint16_t v) { put(a, v); put(a+1, v>>8); }
    ...
    int get_rjmp()   {
        ...
        return (pc + 2*((opcode & 0x800) > 0 ?
            ((opcode & 0x7FF) - 0x800) :
             (opcode & 0x7FF))) & 0x1FFFF;
    }

    int get_branch() {

        return (pc + 2*((opcode & 0x200) > 0 ?
            ((opcode & 0x1F8)>>3) - 0x40 :
            ((opcode & 0x1F8)>>3) )) & 0x1FFFF;
    }

};

// Палитра для DOS 320x200 MODE 13h
static const int doscolor[256] =
{
...
static const uint8_t ansi16[256][16] = {
