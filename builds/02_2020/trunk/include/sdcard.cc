...
protected:
    ...
    // Отправка команды
    void SPI_command(byte cmd_id) {

        outp(SPI_CMD,   cmd_id);
        outp(SPI_LATCH, 0);             // Отключить
        outp(SPI_LATCH, 1);             // Позитивный фронт
        while (inp(SPI_STATUS) & 1);    // Ждать пока не BUSY
        outp(SPI_LATCH, 0);             // Отключить
    }
    ...
    byte SPI_get() {
        ...
        return inp(SPI_DATA);           // Прием полученного ответа
    }
    ...
    byte init() {
        ...
        // Запуск инициализации 74 тактов
        SD_error = SD_OK;
