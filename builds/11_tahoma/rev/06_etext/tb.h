    ...
    App(int argc, char** argv, const char* name = "AVR CPU CORE") {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (argv[i][0] == '-') {
                ...
                if (argv[i][1] == 'd') dbf = fopen("debug.log", "w");
    ...
    void tick() {
        ...
        if (mod_cpu->we) {
            ...
            switch (mod_cpu->address) {
                ...
                case 0x21: font_addr_cu = 256*(font_addr_cu & 0xFF) + mod_cpu->data_o; break;
        ...
        // Чтение из памяти (32K)
        mod_cpu->ir = program [mod_cpu->pc]; // & 0x7FFF

        switch (mod_cpu->address) {

            case 0x20: mod_cpu->data_i = kb_data; break;
            case 0x21: mod_cpu->data_i = fontmem[font_addr_cu & 0xFFF]; break;
        ...
        // Отладка
        if (dbf) { fprintf(dbf, "%05x:%04x [%04x] %02X %c %02X\n", 2*mod_cpu->pc, mod_cpu->address, mod_cpu->ir, mod_cpu->data_i, mod_cpu->we ? 'w' : ' ', mod_cpu->data_o); }
