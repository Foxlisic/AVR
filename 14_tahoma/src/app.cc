...
protected:
    ...
    int sd_data_o, sd_data_i, sd_cmd, sd_cmd_prev, sd_timeout = 1;
...
public:
    ...
    App(int argc, char** argv) {
        ...
        sdl_window          = SDL_CreateWindow("AVR Emulator", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 2*width, 2*height, SDL_WINDOW_SHOWN);
        ...
        program     = (unsigned short*) malloc(65536);
    ...
    void tick() {
        ...
        mod_vga->D  = memory[ mod_vga->A ];

        int addr = mod_cpu->address;

        // Чтение из памяти
        switch (addr) {

            case 0x20: mod_cpu->data_i = banknum; break;
            case 0x21: mod_cpu->data_i = kb_data; break;
            // 0=Kb Latch; 1=Busy SD; 2=Timeout
            case 0x22: mod_cpu->data_i = kb_latch + sd_timeout*4; break;
            case 0x23: mod_cpu->data_i = sd_data_i; break;
            default:   mod_cpu->data_i = memory[ addr ]; break;
        }
        ...
        if (mod_cpu->we) {
            ...
            switch (addr) {
                ...
                case 0x20: banknum  = mod_cpu->data_o;
                case 0x21: mod_vga->BORDER = (mod_cpu->data_o & 7);
                case 0x22: kb_latch  = 0; break;
                case 0x23: sd_data_o = mod_cpu->data_o; break;
                case 0x24: sd_cmd    = mod_cpu->data_o; spi_process(); break;
                default:   memory[ addr ] = mod_cpu->data_o;
            }
        }
        ...
        if (debug) {
            ...
            fprintf(fp, "pc=%04x, %04x / addr=%04x [I:%02x, O:%02x] %c\n",
                2*mod_cpu->pc, mod_cpu->ir, mod_cpu->address, mod_cpu->data_i,
                mod_cpu->data_o, mod_cpu->we ? 'W' : ' ');
        ...
        // Симуляция нажатия на клавиатуру
    ...
    void spi_process() {
        ...
        // Защелка: позитивный фронт получен
        if ((sd_cmd & 0x80) && (sd_cmd_prev & 0x80) == 0)  {
