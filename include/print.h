const byte THEFONT[] PROGMEM =
{
    ,
};

byte _loc_x = 0,  _loc_y = 0;
byte _cl_fr = 7, _cl_bg  = 0;

// Положение курсорума
void locate(byte x, byte y)
{
    _loc_x = x;
    _loc_y = y;
}

// Цвет символов для печати
void color(byte fr, byte bg)
{
    _cl_fr = fr;
    _cl_bg = bg;
}

void printch(byte x, byte y, byte ch)
{
    byte mask;
    for (int i = 0; i < 8; i++) {
        mask = LPM(THEFONT[(ch-32)*8 + i]);
        for (int j = 0; j < 8; j++) {
            pset(x + j, y + i, mask & 0x80 ? _cl_fr : _cl_bg);
            mask <<= 1;
        }
    }
}

// Телетайп-печать строки
void prn(byte ch)
{
    if (ch == 10) {
        _loc_x  = 255;
    } else {
        printch(_loc_x, _loc_y, ch);
    }

    if (_loc_x < 248)
         { _loc_x += 8; }
    else { _loc_x  = 0; _loc_y += 8; }
}

// Печать строки
void print(const char* s)
{
    int i = 0;
    while (s[i]) { prn(s[i]); i++; }
}

// Печать HEX
void printh(word x, byte size = 2)
{
    for (int i = size-1; i >= 0; i--) {
        byte ch = (x >> (4*i)) & 15;
        prn(ch < 10 ? '0' + ch : '7' + ch);
    }
}
